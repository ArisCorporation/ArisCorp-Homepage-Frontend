  ...
  return (
    <div className="w-full">
      <Listbox
        value={state}
        onChange={(event) =>
          changeAction ? changeAction(event) : setState(event)
        }
        disabled={disabled}
      >
        <div className="relative w-full mt-1">
          <motion.div whileTap={animate && { scale: 0.97 }}>
            <Listbox.Button
              className={
                `relative w-full cursor-default rounded-lg bg-${
                  !bg ? '[#111]' : bg
                } py-2 pl-3 pr-10 text-left focus:outline-none focus-visible:border-none focus-visible:ring-0 sm:text-sm ` +
                (disabled && 'opacity-25')
              }
            >
              {mode == 'departments' && (
                <span className="block truncate">
                  {state?.gameplay_name || state || 'Abteilung...'}
                </span>
              )}
              {mode == 'position_level' && (
                <span className="block truncate">
                  {state?.name || 'Positionsstufe...'}
                </span>
              )}
              {mode == 'member' && (
                <span className="block truncate">
          ...
          <Listbox.Options className="z-[60] space-y-2 text-left absolute w-full py-1 pl-0 mt-1 overflow-auto text-base list-none bg-[#111] bg-opacity-75 rounded-md shadow-lg max-h-60 ring-0 focus:outline-none sm:text-sm">
            ...
            {items.map((item, itemIdx) => (
              <Listbox.Option
                key={itemIdx}
                className={({ active }) =>
                  `relative group py-2 cursor-pointer pr-4 list-none select-none ${
                    state ==
                    (mode == 'departments'
                      ? item.gameplay_name
                      : mode == 'position_level'
                    ...
                    !withImages ? 'pl-10 hover:bg-primary hover:text-black' : ''
                  }`
                }
                value={item}
              >
                <div className={withImages && 'flex space-x-2'}>
                  {withImages && (
                    <div
                      className={
                        'w-8 h-8 bg-center bg-no-repeat bg-cover rounded-2xl focus:outline-none group bg-white/5 transition-all duration-200 group-hover:duration-300 ' +
                        (state == item.gameplay_name
                          ? 'grayscale-0'
                          : 'grayscale group-hover:grayscale-0')
                      }
                      style={{
                        backgroundImage: `url(https://cms.ariscorp.de/assets/${item.gameplay_logo.id}?height=400)`,
                      }}
                    />
                  )}
                  <span
