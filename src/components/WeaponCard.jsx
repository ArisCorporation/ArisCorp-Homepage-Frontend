...
const WeaponCard = ({ data, manufacturer }) => (
  <BasicPanel>
    <div className="h-[200px] relative transition-all duration-500 ease">
      <Link href={'/VerseExkurs/waffen/' + data.waffen_name}>
        <a className='relative block w-full h-full overflow-hidden bg-center bg-no-repeat bg-cover group bg-white/5 rounded-2xl' style={{ backgroundImage: `url(https://cms.ariscorp.de/assets/${data.waffen_bild?.id})` }}>
          <div className="absolute bottom-0 w-full pl-4 bg-opacity-80 bg-bg-secondary">
            ...
            <Link href={"/VerseExkurs/firmen/" + data.waffenhersteller?.firmen_name || manufacturer.firmen_name}>
