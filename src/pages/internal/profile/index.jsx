...
export async function getServerSideProps() {
  ...
  const { data: systemData } = await client.query({
    query: GET_VERSEEXKURS_SYSTEME,
  })

  const systemList = systemData.systeme.sort((a, b) =>
...
export default function InternalIndex({ departments, systems, siteTitle }) {
    ...
    if (cropper) {
      const file = await fetch(cropper.getCroppedCanvas().toDataURL())
        ...
        .then((blob) => {
          return new File(
            [blob],
            data.firstname + '-' + data.lastname + '-avatar' + '.png',
            { type: 'image/png' }
          )
        })
      closeModal()
  ...
  useEffect(() => {
    // LOCAL STORAGE
    const ruleValue = window.localStorage.getItem('avatar-rules')
  ...
  useEffect(() => {
    // LOCAL STORAGE
    window.localStorage.setItem('avatar-rules', JSON.stringify(ruleValue))
  }, [ruleValue])
  ...
  useEffect(() => {
    if (data) {
      if (
        ...
        password != '' ||
        recruitingCheckbox !=
          (data.roles?.includes('recruitment') ? true : false) ||
        marketingCheckbox !=
          (data.roles?.includes('marketing') ? true : false) ||
        contentCheckbox !=
          (data.roles?.includes('content_writer') ? true : false) ||
        abteilungsLeiter != data.head_of_department ||
        selectedDepartment?.id !=
          (abteilungsLeiter ? data.head_department[0] : data.department) ||
        sex != data.sex ||
  ...
  async function getData(id) {
    let data = await fetch(
      'https://cms.ariscorp.de/items/member?fields=*,account.email,account.id,account.role,currentResidenceSystem.name,currentResidenceSystem.id,birthSystem.name,birthSystem.id&filter[account][_eq]=' +
        id,
      {
        method: 'GET',
      }
  ...
  function setStates() {
    ...
    if (data.head_of_department) {
      ...
      if (data.head_department)
        setSelectedDepartment(
          departments.find((e) => e.id == data.head_department[0])
        )
    } else if (!data.head_of_department) {
      setAbteilungsleiter(false)
      if (data.department)
        setSelectedDepartment(departments.find((e) => e.id == data.department))
    }
  ...
  async function editMember(edits, id, accId) {
    ...
    if (edits.firstname || edits.lastname)
      accountEdits.email = `${slugify_dot(
        edits.firstname || slugify_dot(firstname)
      )}.${edits.lastname || slugify_dot(lastname)}@ariscorp.de`
  ...
  async function saveMemberEdit() {
    console.log(
      `📝 ------MEMBER EDIT - ${data.title ? data.title : ''} ${
        data.firstname
      } ${data.lastname}------`
    )
    const roles = []
  ...
  return (
    <ProtectedLayout changes={changesMade} helpAction={openHelpModal}>
      <Head>
      ...
      <Modal
        ...
        title={
          ...
          (modalType == 'avatarRules' && 'Avatar Regeln:') ||
      ...
      >
        <div className="mb-2">
          ...
          {modalType == 'avatarRules' && (
            <div className="px-8">
              ...
              <div className="w-full mt-8 space-x-12">
                ...
                <DefaultButton animate agree action={handleRuleAccept}>
                  Akzeptieren!
                </DefaultButton>
              </div>
            </div>
          ...
          {modalType == 'help' && (
            <div className="px-8">
              <div>
                <p>Hier kannst du deinen Charakter anpassen.</p>
                <p>Bitte beachte folgende Dinge:</p>
                <ul className="">
                  <li>Es ist mindestens der Vorname bentötigt.</li>
                  <li>
      ...
      <div className="py-8">
            ...
            <div className="text-[#444] items-center flex space-x-1">
              ...
              <span className="my-auto">
                <Link
                  href={
                    '/VerseExkurs/starmap/' +
                    data.currentResidenceSystem?.name.toLowerCase()
                  }
                  className="transition-all duration-200 opacity-75 decoration-transparent hover:opacity-100 hover:duration-300"
                >
        ...
        <div>
          ...
          <div className="flex flex-wrap justify-between px-8 lg:space-x-12 lg:flex-nowrap">
            ...
            <div className="flex flex-wrap justify-between w-1/2 mb-3 h-fit">
              ...
              <div className="flex justify-between w-full mt-3 mb-3 space-x-4 basis-full lg:w-1/2">
                ...
                <div className="w-8/12 lg:w-full lg:max-w-[380px] justify-center flex">
                  {ruleValue ? (
                    <input
          ...
          <div className="flex mt-12 mb-4">
            <span className="inline-block mr-4 text-xl min-w-fit">
          ...
          <div className="flex flex-wrap justify-between px-8 lg:space-x-12 lg:flex-nowrap">
            ...
            <div className="flex justify-between w-full mb-3 space-x-4 lg:w-1/2">
              <label className="my-auto text-xl">
                Größe <span className="text-sm">(in CM)</span>:
              </label>
              <div className="w-8/12 lg:w-full lg:max-w-[380px]">
                <input
                  value={height}
          ...
          <div className="flex flex-wrap justify-between px-8 lg:space-x-12 lg:flex-nowrap">
            ...
            <div className="flex justify-between w-full mb-3 space-x-4 lg:w-1/2">
              <label className="my-auto text-xl">
                Gewicht <span className="text-sm">(in Kg)</span>:
              </label>
              <div className="w-8/12 lg:w-full lg:max-w-[380px]">
                <input
                  value={weight}
              ...
              >
                <div className="flex flex-wrap justify-between px-8 lg:space-x-12 lg:flex-nowrap">
                  <div className="flex flex-wrap justify-between w-full mb-3 space-x-4">
                    <label className="w-full my-auto text-xl">
                      Warum bist du Bürger?
                    </label>
                    <div className="flex my-auto mr-auto space-x-4">
                      <div>
                        <RadioButton
                          state={citizenReason}
                          setState={setCitizenReason}
                          id="c-10"
                          color="green-700"
                          bg="[#222]"
                          name="citizenReason"
                        ...
                        >
                          <RadioButton.Label>
                            Durch einen Militärischen Dienst
                          </RadioButton.Label>
                          <RadioButton.Indicator />
                        </RadioButton>
                      </div>
                ...
                <div className="flex flex-wrap justify-between px-8 lg:space-x-12 lg:flex-nowrap">
                  ...
                  <div className="flex justify-between w-full mb-3 space-x-4 lg:w-1/2">
                    ...
                    <div className="w-8/12 lg:w-full lg:max-w-[380px]">
                      ...
                      />
                      <p className="text-xs">
                        Wie wurde deine Dienstzeit beendet?
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <AnimatePresence>
            {citizenState == 'citizen' && citizenReason == 'education' && (
              <motion.div
                ...
                className="overflow-hidden"
              >
                <p className="w-full mt-2 ml-4">
                  Informationen zu deinem Bildungsweg:
                </p>
                <div className="flex flex-wrap justify-between px-8 lg:space-x-12 lg:flex-nowrap">
                  <div className="flex justify-between w-full mb-3 space-x-4 lg:w-1/2">
                    ...
                    <div className="w-8/12 lg:w-full lg:max-w-[380px]">
                      ...
                      />
                      <p className="text-xs">
                        Wie heißt deine Ausbildung/Studium
                      </p>
                    </div>
                  </div>
                  <div className="flex justify-between w-full mb-3 space-x-4 lg:w-1/2">
                    ...
                    <div className="w-8/12 lg:w-full lg:max-w-[380px]">
                      ...
                      />
                      <p className="text-xs">
                        In welchem Zeitraum hast du deine Ausbildung/Studiengang
                        absolviert?
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap justify-between w-full px-8 lg:flex-nowrap">
                  <div className="flex justify-between w-full mb-3 space-x-4">
                    ...
                    <div className="w-8/12 lg:w-full">
                      ...
                      />
                      <p className="text-xs">
                        Wo hast du deine Ausbildung/Studiengang absolviert?
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="w-full">
                <Editor
                  ...
                  init={{
                    ...
                    content_style:
                      'body { font-family:nasalization, sans-serif; font-size:14px; color: #f0f6fc; }',
                    skin: 'ArisCorp',
                    block_formats:
                          ...
                          const items = [
                            {
                              ...
                              onAction: () =>
                                editor.insertContent(
                                  '<basicpanel test="test"><div>test</div></basicpanel>'
                                ),
                              // onClick: function() {
        ...
        <div className="sticky w-full bottom-36 lg:bottom-20">
          <div className="absolute flex space-x-4 right-4">
            <XCircleIcon
              onClick={() => changesMade && openCancelModal()}
              className={
                'w-12 h-12 transition duration-200 hover:duration-300 text-red-500/50 hover:text-red-500 ' +
