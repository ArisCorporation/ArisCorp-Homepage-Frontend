...
async function formData() {
  ...
  await Promise.all(
    rawShipMatrixData.map(async (obj) => {
      ...
      // if (flPaints.length > 0 && obj.slug == 'carrack') {
      //   flPaints.forEach(async (i) => {
      //     let backendFiles = await getDirectusFiles()
      //     const fileName = slug + '-' + i.slug
      //     const orgLink = i.storeImage
      //     let fileId
      ...
      //     await backendFiles.forEach(async (file) => {
